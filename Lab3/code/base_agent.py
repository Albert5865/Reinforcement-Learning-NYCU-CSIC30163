		...
		while self.total_time_step <= self.training_steps:
			...
			while True:
				...
				self.gae_replay_buffer.append(0, {
						...
						"reward": reward / 10.0,      # shape = ()
						"value": value.detach().cpu().numpy(),        # shape = ()
						"logp_pi": logp_pi.detach().cpu().numpy()[0],    # shape = ()
	...
	def evaluate(self):
		...
		for i in range(self.eval_episode):
			...
			while True:
				...
				next_observation, reward, terminate, truncate, info = self.test_env.step(action.item())
				total_reward += reward
				if terminate or truncate:
